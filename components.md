...
| `src/screens/MembersScreen.tsx` | 「メンバーを入力」「保存したメンバー」の2タブ（選択中は1つ目のタブ名がその構成名になる）。人数指定（2〜10人）、各メンバーの**名前（任意）**と最寄駅の入力。2人以上の駅が確定するまで次へ進めない |
...
| `src/components/SavedGroups.tsx` | 保存したメンバー構成の一覧（読み込み・削除・全消去）。**localStorageのみ**でサーバには送らない（個人名を含むため）。上限10件、超過時は古いものから押し出す。選ぶと入力タブへ自動で戻り、その構成が選択中になる |
| `src/components/MemberSaveBar.tsx` | 入力タブに置く保存・更新の操作。保存済みを選択している間は「〇〇を選択中」を出し、**その構成の更新（名前の変更を含む）**と別名保存ができる |
