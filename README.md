...
**候補駅の並び順**: 既定は「みんな公平に」です。

```
score(駅) = mean(各メンバーの距離) + k * stddev(各メンバーの距離)   // k = 1.0
```

ばらつきだけで並べると**全員が等しく遠い駅**（例：全員から30km）が最良になってしまうため、
近さ（平均）と公平さ（標準偏差）を足し合わせた複合スコアにしています。
式と `k` の根拠は `functions/src/util.ts` のコメントに書いてあります。
「全体の移動が最小」（合計）と「一番遠い人を優先」（最長）も選べます。
駅カードには `平均 1.5km / 最長 1.8km / ばらつき ±300m` のように、
分散ではなく直感的な標準偏差（距離の単位）で表示します。
...
**同名の駅**（桜井駅は奈良県桜井市と大阪府箕面市に別々に存在します）は、
位置が1km以内のときだけ同じ駅として1件にまとめ、離れていれば両方残します。
「阪急桜井」のような路線名付き、「桜井 箕面」のような地名併記でも絞り込めます。
...
- 1回の検索でのコール数の目安:
  ...
  | 場面 | コール |
  |---|---|
  | 駅名の入力（1人あたり・350msデバウンス後） | Text Search 1回 + Autocomplete 1回 |
  | **駅候補の「さらに表示」を開いたとき** | Autocomplete 1回（20件）。**開かなければ発生しない** |
  | 候補選択 | Place Details 1回（Text Search 経由の候補は緯度経度を持つため0回） |
  | 候補駅の算出 | Nearby Search 1回（距離計算はサーバ内で完結。人数が増えても増えない） |
  | 店舗の取得 | Text Search（駅3 × ジャンル数）+ 写真（表示分） |

  「さらに表示」は**開いたときに1回だけ**追加リクエストします。
  平均予算の取得は既存の Text Search のフィールドマスクを増やすだけなので、**コール数は増えません**
  （返すフィールドが増えるぶん SKU の階層が上がる可能性はあります）。
  10分のメモリキャッシュと350msのデバウンスでコール数を抑えています。
...
アプリ本体の配色は `tailwind.config.js` の `colors` にまとめてあります
（`canvas` 背景 / `surface` カード / `ink` 文字 / `accent` ボタン）。
色名に Tailwind 標準のユーティリティ名（`base` など）を使うと、
同名のユーティリティ（`text-base` はフォントサイズ）と衝突して壊れる点に注意してください。

**アイコンの色を変えるときの対応箇所**（いずれも2つのSVGの両方を直す）:
